...
//Insert into Database
   $("button.sendName").click(function(){
       ...
       if ($("button.sendName").val() == "update"){
            ...
            $.post("implement/i.record.php", { fc_update: fc_update, serial_num: serial_num, cc: cc, label: label, prefix: prefix, issue: issue, title: title, alt_num: alt_num, performers: performers, director: director, solo: solo, date: date, comments: comments, record_id: record_id }, //this is JSON that gets posted
            function(data){
		...
		if ($("button.sendName").val() == "record"){
            ...
            $.post("implement/i.record.php", { fc: fc, serial_num: serial_num, cc: cc, label: label, prefix: prefix, issue: issue, title: title, alt_num: alt_num, performers: performers, director: director, solo: solo, date: date, comments: comments }, //this is JSON that gets posted
            function(data){
                $("#loadName").html(data).slideDown("slow"); // send this data from the PHP echo into the #screen DIV
              //  $("#update").fadeIn("slow");
                $("button.sendName").val('track');
